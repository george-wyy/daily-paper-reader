...
## 摘要
我们提出了 EgoNav，这是一个使人形机器人能够完全通过 5 小时的人类步行数据学习，在多样且未见过的环境中穿行的系统，无需任何机器人数据或微调。该系统利用扩散模型，基于过去轨迹、融合了颜色、深度和语义信息的 360 度视觉记忆，以及来自冻结 DINOv3 骨干网络（用于捕捉深度传感器无法识别的外观线索）的视频特征，来预测合理未来轨迹的分布。一种混合采样方案通过 10 个去噪步骤实现了实时推理，并由收缩时界控制器从预测分布中选择路径。我们通过离线评估验证了 EgoNav，结果显示其在避障和多模态覆盖方面优于基准模型；此外，我们还在未见过的室内外环境中，在 Unitree G1 人形机器人上进行了零样本部署。诸如等待开门、绕过人群以及避开玻璃墙等行为从学习到的先验中自然涌现。我们将发布数据集和训练模型。项目网站：https://egonav.weizhuowang.com
