...
title_zh: 当反馈适得其反：在闭环听觉注意力解码范式中调查神经反馈效应
...
score: 7.5
evidence: 闭环人机交互与实时注意力解码
tldr: 本研究探讨了闭环神经反馈（NFB）对听觉注意力解码（AAD）的影响。通过对19名受试者在开环、听觉反馈、视觉反馈及伪反馈四种条件下进行实验，利用线性解码器和隐马尔可夫模型进行在线解码。研究发现，在单次实验中，听觉神经反馈不仅未能提升解码准确率，反而因增加认知负荷和分心导致性能下降，为未来神经控制助听设备的设计提供了重要参考。
source: biorxiv
selection_source: fresh_fetch
motivation: 旨在验证闭环神经反馈是否能通过人类与机器的实时互动来提高听觉注意力解码的准确性。
method: 采用单次实验范式，对比了开环、听觉增益反馈、视觉反馈及伪听觉反馈四种条件下受试者的解码表现。
result: 离线分析显示听觉反馈条件的准确率显著低于基准线，且受试者反映该反馈增加了分心感和切换注意力的难度。
conclusion: 在单次实验中，不稳定的听觉神经反馈会因增加认知负荷而损害解码性能，未来需更稳定的解码器和长期训练。
---

## 摘要
选择性听觉注意力解码（AAD）能够追踪听者在多个并发说话者中关注的对象，是神经控制助听设备的关键构建模块。虽然假设将 AAD 集成到具有实时神经反馈（NFB）的闭环系统中可以通过神经适应和纠错行为提高解码效果，但这种双向人机交互的短期行为和算法影响仍知之甚少。本研究在涉及 19 名参与者的在线 NFB 单次 AAD 范式中，评估了 NFB 对 AAD 准确性和用户体验的影响。参与者在四种条件下执行了带有强制注意力切换的选择性听力任务：开环（OL）、带听觉增益反馈的闭环（CLA）、带视觉反馈的闭环（CLV），以及与参与者个体神经活动脱钩的伪听觉增益控制（psCLA）条件。AAD 在线执行时，在 5 秒滑动窗口上使用了受试者特异性和受试者无关的线性解码器，随后进行了隐马尔可夫模型后处理。在线分析显示所有条件下的解码性能相当。然而，使用受试者无关解码器的离线事后分析显示，CLA 条件下的 AAD 准确率显著低于 OL 基准。主观上，参与者报告 CLA 显著更分散注意力，且需要更高的切换努力。至关重要的是，对 psCLA 条件的因果分析发现，没有确凿证据表明更高的音频增益能本质上提高解码准确率。我们的结果表明，在具有快速变化反馈线索的单次实验范式中，听觉神经反馈可能会通过增加认知负荷和干扰来降低 AAD 性能。这些发现表明，次优反馈可能会阻碍而非促进学习。我们得出结论，更准确、稳定的解码器以及纵向、多阶段的训练方案可能是闭环听觉注意力系统中实现有益神经反馈效应的必要前提。
