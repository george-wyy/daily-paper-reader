...
tldr: 本研究探讨了面部表情衍生的情感嵌入如何辅助短期人体姿态预测。针对现有模型忽视情感信号的问题，提出了一种轻量级自回归预测世界模型，通过可学习的门控机制融合姿态与情感特征，并利用双层LSTM进行15步滚动预测。实验证明，在情感驱动的运动序列中，归一化门控融合能显著提升预测精度，且反事实实验证实了情感信号的有效性。
source: arxiv
...
motivation: 现有的姿态预测模型主要依赖几何运动线索，往往忽略了影响人体运动动力学的情感信号。
method: 提出一种基于双层LSTM的轻量级自回归预测世界模型，通过可学习的门控机制将姿态关键点与面部情感嵌入进行多模态融合。
result: 实验表明归一化门控融合显著提升了情感驱动序列的预测性能，且反事实扰动实验证实了模型对情感输入的敏感性。
conclusion: 将面部表情情感嵌入集成到轻量级预测世界模型中，是实现情感条件化短期姿态预测的一种可行且有效的方法。
---

## 摘要
短期人体姿态预测在交互系统、辅助机器人以及情绪感知人机交互中发挥着至关重要的作用。虽然目前的轨迹预测模型主要依赖于几何运动线索，但它们往往忽略了影响人体运动动力学的潜在情绪信号。本文研究了源自面部表情的情绪嵌入是否能为短期姿态预测提供辅助条件信号。为了在递归预测设置中进一步评估多模态条件化，我们提出了一种轻量级自回归预测世界模型，可执行15步滚动姿态预测。该框架通过可学习的门控机制将姿态关键点与情绪嵌入相结合，并使用基于双层LSTM架构的循环序列模型执行自回归展开预测。实验在两个小规模姿态-情绪视频数据集上进行：面部表情变化极小的受控运动序列，以及面部表情变化显著的自然情绪驱动运动序列。结果表明，简单的多模态融合并不能持续提高预测精度，而归一化门控融合则显著增强了情绪驱动运动序列的性能。此外，反事实扰动实验表明，预测轨迹对多模态输入的变化表现出可衡量的敏感性，这表明面部表情嵌入是作为辅助条件信号而非冗余特征发挥作用的。总之，这些结果表明，将源自面部表情的情绪嵌入整合到基于轻量级预测世界模型架构的情绪条件短期姿态预测中是一种可行的方法。
