...
tldr: 本研究探讨了实时神经反馈（NFB）对听觉注意力解码（AAD）的影响。研究通过19名受试者在开环、听觉闭环、视觉闭环及伪听觉闭环四种实验条件下进行选择性听力任务。结果发现，与开环基准相比，听觉闭环反馈不仅未能提升性能，反而因增加认知负荷和分心导致解码准确率显著下降。这一发现挑战了闭环反馈必然促进学习的假设，强调了在开发神经控制助听设备时，稳定解码算法和长期训练的重要性。
source: biorxiv
selection_source: fresh_fetch
motivation: 旨在验证实时神经反馈是否能通过神经适应和纠错行为提升听觉注意力解码系统的性能。
method: 采用19名受试者参与的单次实验，对比了开环、听觉增益反馈、视觉反馈及伪听觉反馈四种条件下的AAD表现。
result: 实验发现听觉闭环反馈导致解码准确率显著低于开环基准，且受试者主观感受分心严重、切换注意力费力。
conclusion: 不稳定的听觉反馈在短期内会因增加认知负担而适得其反，实现有效的闭环AAD需要更精确的解码器和多阶段长期训练。
---
