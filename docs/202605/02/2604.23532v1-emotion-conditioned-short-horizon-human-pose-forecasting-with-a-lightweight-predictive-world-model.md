...
tldr: 本研究探讨了面部表情衍生的情感嵌入对短期人体姿态预测的影响。针对现有模型忽视情感信号的问题，提出了一种基于轻量级自回归预测世界模型的框架，利用双层LSTM和可学习门控机制融合姿态与情感特征。实验表明，归一化门控融合能显著提升情感驱动动作的预测精度，证明了情感信号作为辅助条件的有效性。
source: arxiv
...
method: 提出一种基于双层LSTM的轻量级自回归预测世界模型，通过可学习的门控机制融合姿态关键点与面部情感嵌入。
result: 归一化门控融合显著提升了情感驱动运动序列的预测性能，且反事实扰动实验证实了模型对情感输入的敏感性。
