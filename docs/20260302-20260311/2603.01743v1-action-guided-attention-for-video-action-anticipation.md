...
tldr: 针对视频动作预测中现有Transformer模型过度依赖像素级视觉线索而缺乏高层语义建模的问题，本文提出动作引导注意力（AGA）机制。该方法利用预测的动作序列作为查询和键来引导序列建模，通过门控函数融合历史关键时刻与当前帧信息。在EPIC-Kitchens-100基准测试中，AGA展现了更强的泛化能力和可解释性，支持对动作依赖关系和反事实证据的深入分析。
selection_source: fresh_fetch
motivation: 现有的基于Transformer的方法由于缺乏高层语义，容易过度拟合过去帧的显式视觉线索，难以捕捉潜在的动作意图。
method: 提出动作引导注意力（AGA）机制，利用预测的动作序列作为查询和键来引导模型关注过去的相关时刻，并通过门控函数融合特征。
result: 在EPIC-Kitchens-100基准测试中，该方法在验证集到未见测试集的泛化表现优异，并提供了透明的可解释性分析。
conclusion: AGA通过引入动作引导的语义建模，不仅提升了动作预测的泛化性能，还为理解模型捕获的动作依赖关系提供了有效手段。
---

## 摘要
预测视频中的未来动作具有挑战性，因为观察到的帧仅提供了过去活动的证据，需要推断潜在意图来预测即将发生的动作。现有的基于 Transformer 的方法依赖于像素表示的点积注意力，往往缺乏对视频序列建模以进行有效动作预测所需的高层语义。因此，这些方法往往会过拟合过去帧中存在的显式视觉线索，限制了它们捕捉潜在意图的能力，并降低了对未见样本的泛化性能。为解决这一问题，我们提出了动作引导注意力（Action-Guided Attention, AGA），这是一种显式利用预测动作序列作为查询（queries）和键（keys）来引导序列建模的注意力机制。我们的方法促使注意力模块根据即将发生的活动强调过去的相关时刻，并通过专门的门控函数将这些信息与当前帧嵌入相结合。AGA 的设计使得能够对从训练集中发现的知识进行训练后分析。在广泛采用的 EPIC-Kitchens-100 基准测试上的实验表明，AGA 从验证集到未见测试集都具有良好的泛化能力。训练后分析可以进一步检查模型捕捉到的动作依赖关系及其内化的反事实证据，为其预测性预报提供透明且可解释的见解。
