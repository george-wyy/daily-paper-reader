...
tldr: 本研究针对多模态意图识别中复杂语义建模和推理能力不足的问题，提出了HIER方法。该方法结合多模态大语言模型（MLLM），通过构建从局部标记到语义概念再到高阶关系的层次化语义表示，并引入基于思维链的进化推理机制，实现了对人类意图的精准捕捉。实验证明，HIER在多个基准测试上均优于现有最先进方法。
selection_source: fresh_fetch
motivation: 现有方法难以建模复杂意图背后的层次化语义，且缺乏对多模态表示进行自我进化推理的能力。
method: 提出HIER方法，通过标签引导聚类和JS散度筛选构建三层语义结构，并利用MLLM的反馈机制进行动态自我进化推理。
result: "在三个挑战性基准测试中，HIER在所有指标上均一致优于现有SOTA方法和MLLM，提升幅度达1-3%。"
conclusion: 层次化语义表示与进化推理的结合能显著提升多模态意图识别的准确性，为复杂对话系统提供了有效的解决方案。
---
