...
title_zh: FEEL (力增强型第一视角学习)：一种用于物理动作理解的数据集
...
tldr: FEEL是首个结合力测量与第一视角视频的大规模数据集，包含约300万帧自然厨房操作场景。研究通过定制压阻手套获取力数据，并将其作为物理交互理解的关键原语。该数据集在接触理解（时间与像素级分割）和自监督动作表示学习任务中表现优异，显著提升了在多个主流动作理解基准上的迁移性能。
selection_source: fresh_fetch
motivation: 物理力是驱动交互的核心原因，但现有第一视角视频数据集缺乏大规模、同步的力测量数据来深入理解物理动作。
method: 开发了定制压阻手套以实现可扩展的数据采集，并利用力预测作为自监督预训练目标来增强视频骨干网络的表示学习。
result: 在无需手动标注的情况下，实现了最先进的时间接触分割效果，并在EPIC-Kitchens等多个动作理解任务中超越了现有主流数据集的迁移表现。
conclusion: FEEL数据集证明了力信息在增强物理动作理解和自监督表示学习方面的巨大潜力，为多模态感知提供了新方向。
---

## 摘要
我们介绍了 FEEL (Force-Enhanced Egocentric Learning，力增强型第一视角学习)，这是首个将定制压阻手套采集的力测量数据与第一视角视频配对的大规模数据集。我们的手套实现了可扩展的数据采集，FEEL 包含约 300 万帧在厨房环境中自然、无脚本操作的力同步帧，其中 45% 的帧涉及手物接触。由于力是驱动物理交互的根本原因，它是物理动作理解的关键原语。我们通过将 FEEL 应用于两类任务，展示了力在物理动作理解中的效用：(1) 接触理解，即联合执行时间接触分割和像素级接触物体分割；(2) 动作表示学习，其中力预测作为视频骨干网络的自监督预训练目标。我们在无需任何手动接触物体分割标注的情况下，实现了最先进的时间接触分割结果和具有竞争力的像素级分割结果。此外，我们证明了利用 FEEL 进行的动作表示学习在无需任何手动标签的情况下，在 EPIC-Kitchens、SomethingSomething-V2、EgoExo4D 和 Meccano 等数据集上的动作理解任务迁移性能得到了提升。
