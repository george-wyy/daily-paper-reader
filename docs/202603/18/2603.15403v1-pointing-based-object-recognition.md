...
motivation: 随着人机交互向直观化发展，识别非言语交流中的指向目标变得至关重要。
method: 整合了目标检测、人体姿态估计、单目深度估计和视觉语言模型，构建了一个多模块识别流水线。
result: 实验表明，结合单目深度信息能显著提高复杂及重叠场景下的目标识别精度。
conclusion: 该模块化系统证明了在仅使用RGB图像的情况下，通过3D空间重建也能实现高效的指向目标识别。
---
