...
title_zh: 基于从选定经验中学习的主动智能体和 DDQN 算法的冻结步态预测
...
tldr: 针对帕金森病患者的步态冻结（FOG）问题，本研究提出了一种基于强化学习的预测框架。通过结合双重深度Q网络（DDQN）和优先经验回放（PER）机制，该模型能够精准识别FOG发生前的关键时间点。实验表明，该方法在受试者相关和无关场景下均能实现约8秒的提前预测，为可穿戴辅助设备的实时干预提供了重要技术支持。
...
## 摘要
冻结步态 (FOG) 是帕金森病 (PD) 患者常见的致残性运动症状，常导致跌倒和活动能力下降。及时准确地预测 FOG 发作对于通过辅助技术实现主动干预至关重要。本研究提出了一种基于强化学习的框架，旨在识别最佳的 FOG 发生前时间点，从而延长预测性提示系统的预测时界。该模型采用了增强了优先经验回放 (PER) 的双深度 Q 网络 (DDQN) 架构，使智能体能够专注于高影响经验的学习并优化其策略。通过 9000 轮训练并采用促进谨慎决策的奖励塑造策略，该智能体在受试者相关和受试者无关的评估中均表现出稳健的性能。在受试者无关场景下，该模型在 FOG 发生前实现了高达 8.72 秒的预测时界，在受试者相关设置下为 7.89 秒。这些结果突显了该模型集成到可穿戴辅助设备中的潜力，为缓解 PD 患者的 FOG 提供及时且个性化的干预。
