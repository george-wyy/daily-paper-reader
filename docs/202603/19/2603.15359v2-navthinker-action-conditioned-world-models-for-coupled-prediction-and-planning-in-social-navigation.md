...
## 摘要
社交导航要求机器人在动态的人类环境中安全行动。有效的行为需要“前瞻性思考”：即推理场景和行人在不同机器人动作下如何演变，而非仅仅对当前观测做出反应。这产生了一个耦合的预测-规划挑战，其中机器人动作与人类运动相互影响。为了应对这一挑战，我们提出了 NavThinker，这是一个将动作条件世界模型与同策略强化学习相结合的未来感知框架。该世界模型在 Depth Anything V2 的补丁特征空间中运行，并对未来场景几何和人类运动进行自回归预测；随后，多头解码器生成未来深度图和人类轨迹，从而产生与可通行性和交互风险相一致的未来感知状态。至关重要的是，我们使用 DD-PPO 训练策略，同时通过以下方式注入世界模型的前瞻信号：(i) 将动作条件的未来特征融合到当前观测嵌入中，以及 (ii) 根据预测的人类轨迹进行社交奖励塑造。在单机器人和多机器人 Social-HM3D 上的实验表明，该方法取得了最先进的导航成功率，并能零样本迁移到 Social-MP3D 以及在 Unitree Go2 上进行实机部署，验证了其泛化能力和实际应用性。网页：https://hutslib.github.io/NavThinker。
