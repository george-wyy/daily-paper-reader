...
tldr: 针对情感支持对话系统中专家定义奖励信号稀疏且缺乏解释性的问题，本文提出RAPO框架。该框架将对话视为反应驱动过程，利用模拟用户反应生成稠密的自然语言反馈。通过事后对话选择关键轮次、生成对比排名信号与自然语言评论，并结合标量奖励与语言反馈进行混合策略优化，实现了更精准的情感对齐。实验证明RAPO在提升用户情感状态方面显著优于传统强化学习基准。
selection_source: fresh_fetch
motivation: 传统情感支持系统依赖稀疏的专家评分，难以解释响应失败原因或捕捉动态的用户情感变化。
method: 提出RAPO框架，利用事后对话选择、生成式反馈以及标量与语言反馈结合的混合策略优化模型。
result: 在ESC和Sotopia基准测试中，RAPO在驱动积极交互结果和情感改善方面均显著优于强强化学习基准。
conclusion: 将用户反应转化为细粒度的语言反馈并与标量奖励结合，是提升对话系统情感支持能力的有效途径。
---
