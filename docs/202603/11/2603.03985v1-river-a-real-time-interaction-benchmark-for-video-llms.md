...
score: 7.0
evidence: 实时交互与前瞻性预测基准
tldr: 针对现有视频大语言模型多处于离线模式、缺乏实时交互能力的问题，本文提出了RIVER Bench基准测试。该基准包含回顾性记忆、实时感知和前瞻性预测三大任务，模拟真实对话场景。研究发现离线模型在实时处理上表现欠缺，为此提出了一种通用的改进方法，增强了模型的长时记忆与未来感知能力，显著推动了实时视频理解领域的发展。
selection_source: fresh_fetch
motivation: 现有的视频大语言模型大多采用离线处理模式，无法满足实时交互的需求。
method: 提出了包含回顾性记忆、实时感知和前瞻性预测任务的RIVER Bench，并设计了一种通用的实时交互改进方法。
result: 评估显示离线模型在单次问答中表现尚可，但在实时处理、长时记忆和未来感知方面存在明显短板。
conclusion: RIVER Bench为实时视频交互提供了评估标准，所提改进方法有效提升了模型在动态环境下的交互灵活性。
---
