...
score: 6.0
evidence: 概率建模与实时推理
tldr: 本文提出一种用于非线性时变系统状态与参数在线联合估计的滤波框架。该方法利用Rao-Blackwellization技术，通过卡尔曼滤波解析计算条件状态分布，并采用Stein变分梯度下降（SVGD）近似包括噪声协方差在内的模型参数。研究不仅证明了算法的理论一致性，还通过生物反应器和神经网络增强系统的案例，展示了其在实时推理和在线神经网络训练中的高效性与稳定性。
selection_source: fresh_fetch
motivation: 旨在解决非线性时变系统中状态与参数（特别是噪声协方差）在线联合估计的计算效率与稳定性挑战。
method: 结合Rao-Blackwellization技术，利用卡尔曼滤波处理状态分布，并采用基于粒子的SVGD方法对模型参数进行变分推断。
result: 理论上给出了参数近似误差对状态估计影响的一致性界限，并在生物反应器和神经网络动态系统实验中验证了优越性能。
conclusion: 该框架实现了高效的实时联合推理，特别是在动态模型中在线训练神经网络方面展现了强大的数据驱动系统辨识潜力。
---

## 摘要
我们提出了一种用于非线性时变系统中在线联合状态估计和参数辨识的滤波框架。该算法利用 Rao-Blackwell 化技术高效地推断状态-参数联合后验。具体而言，条件状态分布通过卡尔曼滤波进行解析计算，而包括过程和测量噪声协方差在内的模型参数则使用基于粒子的 Stein 变分梯度下降（SVGD）进行近似，从而实现稳定的实时推理。我们通过界定 SVGD 近似参数后验对状态估计的影响，证明了一个理论一致性结果，将真实参数后验与近似参数后验之间的散度与所得状态边缘分布之间的全变分距离联系起来。所提滤波器的性能在两个案例研究中得到了验证：一个具有 Haldane 动力学的生物反应器和一个神经网络增强的动力系统。后者展示了该滤波器在动力学模型中进行在线神经网络训练的能力，体现了其在全自适应、数据驱动系统辨识方面的潜力。
