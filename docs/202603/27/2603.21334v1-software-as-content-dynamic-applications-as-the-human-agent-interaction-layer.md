...
motivation: 现有的聊天界面在处理结构化数据、高熵输入以及缺乏持久交互状态方面存在固有局限。
method: 提出 SaC 范式，利用动态生成的、具有状态且不断进化的应用程序代替传统文本响应作为人机交互媒介。
result: 在选择、探索和执行等典型任务中证明了该系统的技术可行性，并明确了其相对于纯自然语言交互的优势与边界。
conclusion: 将界面重构为动态生成的软件工件，为人类与 AI 的交互开辟了全新的设计空间，使动态软件成为可研究的实体。
---
